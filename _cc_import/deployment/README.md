...
Цей runbook розгортає release `1.1.0` без автоматичної активації операцій.
...
- встановлений `erpnext_ua` 0.3.0+ із українським планом №291 або №186;
...
bench --site <staging-site> install-app erpnext_consignment_and_commission
...
```bash
cd apps/erpnext_consignment_and_commission
git fetch origin
...
```bash
bench --site <staging-site> execute \
  erpnext_consignment_and_commission.consignment_and_commission.integrations.reconciliation.audit_financial_integrity
```
