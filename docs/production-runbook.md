...
2. Відкрийте `UA Tax Parameters` поточного року. Має бути повний комплект груп
   1–3, дата перевірки й офіційні джерела. Не замінюйте в ньому законодавчий
   максимум локальною ставкою.
3. Для кожного `FOP Profile` групи 1–2 окремо вкажіть фактичну суму ЄП, рік,
   дату перевірки й URL рішення громади, а тоді згенеруйте календар. Звірте дати
   з актуальним календарем ДПС, особливо виняткові неопераційні дні.
4. Створіть `UA KEP Key`, перевірте власника та `Valid Until`.
5. Створіть `PRRO Cash Register`: фіскальний номер, локальний номер реєстратора
...
6. Для кожного `Mode of Payment` вкажіть `PRRO Payment Form Code`.
7. Для ПДВ/акцизу заповніть у `Sales Taxes and Charges` PRRO Type/Letter/Name,
   а в Item — `PRRO Tax Letters`; УКТЗЕД і ДКПП заповнюйте там, де це вимагає закон.
8. Створіть `POS Printer` з приватною IP, портом 9100, code page принтера
...
9. Прив'яжіть до каси склад, клієнта за замовчуванням, ПРРО, КЕП і банківський термінал.
10. Надайте співробітникам `Employee Cash Desk Access`; права адміністратора касиру не давайте.
