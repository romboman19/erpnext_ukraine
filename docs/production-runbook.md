...
2. Відкрийте `UA Tax Parameters` поточного року. Має бути повний комплект груп
   1–3, дата перевірки й офіційні джерела. Для ФОП груп 1–2 замініть максимум ЄП
   фактичною ставкою з рішення громади, після чого повторно згенеруйте календар.
   Звірте дати з актуальним календарем ДПС, особливо виняткові неопераційні дні.
3. Створіть `UA KEP Key`, перевірте власника та `Valid Until`.
4. Створіть `PRRO Cash Register`: фіскальний номер, локальний номер реєстратора
...
5. Для кожного `Mode of Payment` вкажіть `PRRO Payment Form Code`.
6. Для ПДВ/акцизу заповніть у `Sales Taxes and Charges` PRRO Type/Letter/Name,
   а в Item — `PRRO Tax Letters`; УКТЗЕД і ДКПП заповнюйте там, де це вимагає закон.
7. Створіть `POS Printer` з приватною IP, портом 9100, code page принтера
...
8. Прив'яжіть до каси склад, клієнта за замовчуванням, ПРРО, КЕП і банківський термінал.
9. Надайте співробітникам `Employee Cash Desk Access`; права адміністратора касиру не давайте.
