# GSF (Group Stock FIFO) — передача агенту, стан на 2026-07-31
...
└── release/
    ...
    ├── phase-5.md                ← звіт по Phase 5, продаж і компенсація
    └── phase-6-8.md              ← сага, повернення, інвентаризація, період
```
...
│   ├── gsf_checkout/             ┐ Phase 5 (сага §9.16)
│   ├── gsf_checkout_line/        ┘
│   ├── gsf_physical_stock_count/ ┐ Phase 7 (§20.3)
│   └── gsf_stock_count_line/     ┘
├── services/
...
│   ├── compensation.py           ← §23.2 реверс закоміченого (Phase 5)
│   ├── sale.py                   ← §18.2 продаж + §16.4 перевірка COGS
│   ├── checkout_states.py        ← ЧИСТА стан-машина §23
│   ├── checkout.py               ← сага: reserve → prepare → sell → compensate
│   ├── returns.py                ← §19 повернення покупця
│   ├── integrity.py              ← §31 Financial Integrity
│   └── period.py                 ← §25 backdated guard і закриття періоду
├── setup/
...
│   ├── test_checkout_states.py    ← 19 тестів (сага §23)
...
│   ├── phase_5_checks.py          ← прогін Phase 5: компенсація, потім продаж
│   └── phase_5_saga.py            ← прогін саги: abort і повний продаж
...
## 6d. Що ЗРОБЛЕНО в Phase 5 — managed sale

Детальний звіт — `docs/group-stock-fifo/release/phase-5.md`. Коротко:

**Готово:** компенсація §23.2 (залишок Phase 4), Sales Invoice з технічними
рядками §18.2, перевірка COGS §16.4, readiness на `allow_multiple_items` §18.3
і на непрацюючий scheduler.

**Три речі, які варто знати про цей код:**
1. **Rollback і компенсація — різні операції.** До коміту — savepoint
   (ADR-008). Після — тільки нові документи у зворотному напрямку: скасування
   переписало б історію, якої дві компанії вже торкнулися.
2. **Що рухати — читається з книги, не з плану.** До моменту компенсації
   allocation може вже не існувати; питання стоїть про те, що зробили
   документи.
3. **§16.4 кидає, а не звітує.** Після появи фіскального чека єдина корекція —
   юридичне повернення, тож розбіжність має зупинити транзакцію.

**Доведено живцем:** компенсація повернула всі три пули точно в попередній стан
і розкрутила clearing-рахунки **в нуль**; наступний продаж дав **COGS
6 500.00** — рівно §37.1 — трьома технічними рядками 2/3/1.

---

## 6e. Що ЗРОБЛЕНО в Phases 6–8 — сага, повернення, період

Детальний звіт — `docs/group-stock-fifo/release/phase-6-8.md`. Коротко:

**Готово:** `GSF Checkout` із 17 станами §23.1 і ідемпотентним відновленням;
повернення §19 з новим шаром і карантином; `GSF Physical Stock Count` §20.3;
backdated guard і закриття періоду §25; Financial Integrity §31; scheduled jobs;
усі 17 runbooks §45.

**Три речі, які варто знати про цей код:**
1. **Сага існує заради однієї асиметрії.** До інвойсу збій відкочується (§14.6).
   Після — може існувати фіскальний чек, до якого rollback не дістає, тож треба
   **розвертати, а не відкочувати** (§23.2). Уся її робота — знати, з якого
   боку межі вона зараз.
2. **Сага комітить між кроками, і це не порушення §14.6.** §13.1 хоче, щоб
   резервування починало власну транзакцію; §23 хоче, щоб сагу можна було
   знайти після падіння. Підготовка й продаж лишаються в одній транзакції.
3. **Інвентаризація не проводить коригувань свідомо** — §20.2 забороняє
   автоматичний розподіл, а питання «кого не вистачає» вирішує, чиї книги
   приймають списання.

**Доведено живцем:** кошик до кінця — `COMPLETED`, три технічні рядки, **COGS
6 500**; кошик, зупинений на `STOCK_PREPARED` і скасований — `COMPENSATED`, усі
три пули повернулись точно; звіт цілісності на позитивному контролі дав
**2 CRITICAL** саме на те, що було зігнуто; інвентаризація відтворила форму
§20.1 з різницею −1; закриття періоду «назад» відхилено.

---

## 7. Що лишилось до production

Усі вісім фаз §41 пройдені. Те, що нижче, — не «недороблені фази», а конкретні
речі, кожна з власною причиною, чому її ще немає.

### Потребує фікстури трекінгового товару

На тестовому сайті **немає жодного Batch- чи Serial-товару**, тож три речі
написані, але не доведені живим прогоном:

- **Serial allocation** — зараз чесно fail-closed (`SERIAL_AMBIGUOUS`).
  Потрібна поштучна правда з книги про те, які серійники шару ще на складі.
- **Приймання Batch/Serial** (§11.2) — код читає `Serial and Batch Bundle` і
  відмовляє, якщо один рядок несе кілька партій; прогону не було.
- **Карантин повернень** (§19.3) — маршрут написаний, товару для нього немає.

Це найдешевший спосіб закрити одразу три дірки.
...
- **Проводка коригувань інвентаризації** — §20.2 забороняє автоматичний
  розподіл і робить `MANUAL_APPROVAL` дефолтом. Документ рахує різницю по
  доменах; проводка робиться руками.
- **`GSF Opening Stock Import` (§38.2)** — форма залежить від того, як власник
  вивантажить наявні залишки. Тимчасовий шлях описаний у
  `runbooks/opening-stock-migration.md`.
- **Item policy (§12.2)** — DocType політики товару в моделі §9 немає; запит
  несе `item_policy` і зберігає снапшот, правил поверх нього ще нема.
- **`GSF Global Revaluation` (§25.3)** — прямо поза MVP за ADR-010.
- **Counterparty accounting dimension** — ERPNext 16 відмовляється створювати
  dimension над `Company` (амендмент ADR-005). Звірка йде через
  `GSF Reallocation Leg.counterparty_company`.

### Ще не написано

- **§18.4 контрольований renderer друку** — `gsf_display_group` на рядках це
  саме той ключ, який йому потрібен.
- **CC compatibility suite (§37.24–37.29)** — жодного інтеграційного тесту
  GSF+CC разом. Домени розділені `GSF Warehouse Binding` і на сайті не
  перетинаються, але тесту, який це стереже, немає.
- **Workspace / UI** — жодного екрана; усе через API і desk-форми.

---

## 8. Одинадцять пасток, у які я вже вступив — щоб ви не вступали повторно
...
1. **Фікстура трекінгового товару.** Найдешевше і закриває найбільше: serial
   allocation, приймання Batch/Serial і карантин повернень зараз написані, але
   не прогнані. Один Batch-товар і один Serial-товар у фікстурі §37.1.
2. **Прогін на `frappe-uat`** — там є scheduler. Дасть відразу дві речі:
   перевірку самих scheduled job і зникнення `PENDING_REPOST`, який на
   `frappe-test` доводиться зливати руками.
3. **CC compatibility suite (§37.24–37.29).** Домени розділені біндингом і на
   сайті не перетинаються — але тесту, який це стереже, немає, а саме він
   ловить регресію, коли комісійний модуль зміниться.
4. **Навантаження й deadlock (§41 Phase 8)** — після кроку 2, на стеку зі
   scheduler.
...
Перед будь-яким із цих кроків прочитайте розділ 8: одинадцять пасток — це
одинадцять помилок, які вже коштували часу, і майже всі вони виглядали
розумними, поки не спрацювали.
