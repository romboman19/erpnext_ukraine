...
### Phase 5 — залишок (НАСТУПНИЙ КРОК)
- **`GSF Checkout` (§9.16, §23.1)** — сага з 17 станів. Усі кроки, які вона має
  оркеструвати, існують і доведені окремо: `reserve` → `prepare` → `sell`, плюс
  `compensate` для збою. Лишилось звести їх у персистентну стан-машину з
  ідемпотентним відновленням.
- §31.6 Financial Integrity — дані вже в `GSF Reallocation Leg`; прогін Phase 5
  показав, що після компенсації due-from і due-to сходяться в нуль.
- §18.4 контрольований renderer друку — `gsf_display_group` на рядках це саме
  той ключ, який йому потрібен.

### Phase 6 — POS/PRRO (після саги)
- Інтеграція з фіскалізацією. `POS Order` вже існує в `erpnext_ua` (не GSF), і
  ADR-012 каже, що фіскалізацію ІНІЦІЮЄ `POS Order`, GSF туди не лізе — сага
  `GSF Checkout` має бути маршрутом ПІД ним, а не окремим життєвим циклом.
...
- CC compatibility suite (§37.24–37.29) — жодного інтеграційного тесту
  GSF+CC разом.
- §18.4 контрольований renderer друку.
- Фікстура трекінгового товару: закриває дірки одразу в Phase 2 (приймання на
  Batch/Serial) і Phase 3 (serial allocation, зараз fail-closed).
