...
### Автоматизовано після первинної передачі

- **CC compatibility (§37.24–37.29):** clean-site CI проводить змішаний
  GSF/consignment/commission продаж і повернення, перевіряє lineage обох
  доменів, фізичні totals 2 + 2 та відмову `CC_WAREHOUSE_CONFLICT`.
- **CC discovery (§8.3):** `CC Location` автоматично підтримує read-only
  `CC_* / DISCOVERED_EXTERNAL` bindings; migrate відновлює drift, readiness
  блокує конфлікт.
