...
class VitalPBXClient:
    ...
    def health(self) -> dict:
        r = requests.get(f"{self.base_url}/api/ping", headers=self._headers(), timeout=self.timeout)
        r.raise_for_status()
    ...
    def click_to_call(self, extension: str, destination: str) -> dict:
        ...
        r = requests.post(f"{self.base_url}/api/click2call", headers=self._headers(), json=payload, timeout=self.timeout)
        r.raise_for_status()
        return r.json() if (r.text or "").strip() else {"ok": True}
