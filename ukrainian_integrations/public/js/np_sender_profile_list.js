frappe.listview_settings['NP Sender Profile'] = {
  onload(listview) {
    listview.page.add_menu_item('Створити ТТН (з профілю)', async () => {
      ...
      const d = new frappe.ui.Dialog({
        ...
        primary_action: async (v)=>{
          const x=await frappe.call({method:'ukrainian_integrations.shipment.nova_poshta.service.create_ttn_from_sales_invoice', args:v});
          frappe.msgprint('ТТН: '+((x.message||{}).ttn_number||'-')); d.hide();
        }
      });
      d.show();
    });
