...
def create_ttn_standalone(
    ...
    return {
        ...
        "ttn_ref": row.get("Ref") or "",
        "print_url": row.get("Ref") and f"https://my.novaposhta.ua/orders/printMarking100x100/orders[]/{row.get('Ref')}/type/pdf/apiKey/{profile_api_key}",
        "sticker_url": row.get("Ref") and f"https://my.novaposhta.ua/orders/printMarking100x100/orders[]/{row.get('Ref')}/type/pdf/apiKey/{profile_api_key}",
        "raw": row,
    }
