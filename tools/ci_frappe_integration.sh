...
APP_DIR="$BENCH_DIR/apps/erpnext_ua"
...
runuser -u frappe -- bash -lc "cd '$BENCH_DIR' && ./env/bin/pip install -e apps/erpnext_ua"
runuser -u frappe -- bash -lc "cd '$BENCH_DIR' && echo >> sites/apps.txt && echo erpnext_ua >> sites/apps.txt"
...
runuser -u frappe -- bash -lc "cd '$BENCH_DIR' && bench --site ci.localhost install-app erpnext_ua"
runuser -u frappe -- bash -lc "cd '$BENCH_DIR' && bench --site ci.localhost migrate"
runuser -u frappe -- bash -lc "cd '$BENCH_DIR' && bench --site ci.localhost migrate"
runuser -u frappe -- bash -lc "cd '$BENCH_DIR' && bench --site ci.localhost execute erpnext_ua.integrations.diagnostics.run_installation_checks"
runuser -u frappe -- bash -lc "cd '$BENCH_DIR' && bench --site ci.localhost set-config allow_tests true"
runuser -u frappe -- bash -lc "cd '$BENCH_DIR' && bench --site ci.localhost run-tests --app erpnext_ua"
