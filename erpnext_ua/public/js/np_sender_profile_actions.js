    ...
    frm.add_custom_button('Створити ТТН (довільно)', async () => {
      ...
      const d = new frappe.ui.Dialog({
        ...
        primary_action: async (v) => {
          const r = await frappe.call({ method: 'erpnext_ua.integrations.shipment.nova_poshta.service.create_ttn_standalone', args: { ...v, idempotency_key: idempotencyKey, sender_profile: frm.doc.profile_name || frm.doc.name, sender_branch: ((v.sender_branch||"").split("::")[0] || "") } });
          const m = (r.message || {});
      ...
      try {
        const br = await frappe.call({ method: 'erpnext_ua.integrations.shipment.nova_poshta.service.np_sender_branches', args: { sender_profile: frm.doc.profile_name || frm.doc.name } });
      ...
      d.get_field('settlement_query').$input.on('change', async () => {
        ...
        const r = await frappe.call({ method:'erpnext_ua.integrations.shipment.nova_poshta.service.np_search_settlements', args:{ query:q, sender_profile: frm.doc.profile_name || frm.doc.name }});
      ...
      d.get_field('warehouse_query').$input.on('change', async () => {
        ...
        const r = await frappe.call({ method:'erpnext_ua.integrations.shipment.nova_poshta.service.np_search_warehouses', args:{ settlement_ref, query, sender_profile: frm.doc.profile_name || frm.doc.name }});
        const items = (r.message && r.message.items) || []; if (!items.length) return;
