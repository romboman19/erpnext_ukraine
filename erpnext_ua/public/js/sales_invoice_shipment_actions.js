  ...
  refresh(frm) {
    ...
    frm.add_custom_button('NP: Створити ТТН', async () => {
        ...
        primary_action: async (values) => {
          try {
            const r = await frappe.call({
              method: 'erpnext_ua.integrations.shipment.nova_poshta.service.create_ttn_from_sales_invoice',
              args: {
      ...
      sq.$input.on('change', async () => {
        ...
        try {
          const r = await frappe.call({
            method: 'erpnext_ua.integrations.shipment.nova_poshta.service.np_search_settlements',
      ...
      wq.$input.on('change', async () => {
        ...
        try {
          const r = await frappe.call({
            method: 'erpnext_ua.integrations.shipment.nova_poshta.service.np_search_warehouses',
      ...
      const d = new frappe.ui.Dialog({
        ...
        primary_action: async (values) => {
          try {
            ...
            const r = await frappe.call({
              method: 'erpnext_ua.integrations.shipment.ukr_poshta.service.create_shipment_from_sales_invoice',
              args: {
                sales_invoice: frm.doc.name,
      ...
      frm.add_custom_button('Створити відправлення', async () => {
        ...
        try {
          const response = await frappe.call({
            method: 'erpnext_ua.integrations.shipment.rozetka_delivery.service.rz_sender_profiles_list',
        ...
        const searchCities = async () => {
          ...
          const response = await frappe.call({
            method: 'erpnext_ua.integrations.shipment.rozetka_delivery.service.rz_search_cities',
            args: {
              query,
        ...
        const searchDepartments = async () => {
          ...
          const response = await frappe.call({
            method: 'erpnext_ua.integrations.shipment.rozetka_delivery.service.rz_search_departments',
            args: {
              city_id: cityId,
        ...
        dialog = new frappe.ui.Dialog({
          ...
          primary_action: async values => {
            const response = await frappe.call({
              method: 'erpnext_ua.integrations.shipment.rozetka_delivery.service.create_track_from_sales_invoice',
              args: {
                sales_invoice: frm.doc.name,
    ...
    if (frm.doc.rz_track_id) {
      frm.add_custom_button('Оновити статус', async () => {
        const response = await frappe.call({
          method: 'erpnext_ua.integrations.shipment.rozetka_delivery.service.sync_one_invoice_status',
      ...
      frm.add_custom_button('Завантажити етикетку', () => {
        const url =
          '/api/method/erpnext_ua.integrations.shipment.rozetka_delivery.service.download_track_label' +
