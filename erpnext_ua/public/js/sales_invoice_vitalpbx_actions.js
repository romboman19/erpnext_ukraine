frappe.ui.form.on('Sales Invoice', {
  refresh(frm) {
    ...
    frm.add_custom_button('📞 VitalPBX: Подзвонити клієнту', async () => {
      let defaultExt = '';
      try {
        const ex = await frappe.call({ method: 'erpnext_ua.integrations.pbx_sms.vitalpbx.service.get_default_extension' });
      ...
      const d = new frappe.ui.Dialog({
        ...
        primary_action: async (v) => {
          await frappe.call({
            method: 'erpnext_ua.integrations.pbx_sms.vitalpbx.service.click_to_call',
            args: {
