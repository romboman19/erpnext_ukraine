frappe.listview_settings['NP Sender Profile'] = {
  onload(listview) {
    listview.page.add_menu_item('Створити ТТН (довільно)', async () => {
      ...
      const r = await frappe.call({ method: 'erpnext_ua.integrations.shipment.nova_poshta.service.np_sender_profiles_list' });
      ...
      const d = new frappe.ui.Dialog({
        ...
        primary_action: async (v) => {
          const x = await frappe.call({ method: 'erpnext_ua.integrations.shipment.nova_poshta.service.create_ttn_standalone', args: { ...v, idempotency_key: idempotencyKey } });
