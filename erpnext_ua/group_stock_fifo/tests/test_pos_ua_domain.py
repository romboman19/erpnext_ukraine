...
def _row(name: str, qty: float) -> SimpleNamespace:
    return SimpleNamespace(name=name, qty=qty)


def _request(pos_row: str, qty: float) -> SimpleNamespace:
    return SimpleNamespace(return_against_item=pos_row, qty=qty)
...
class ReturnSlicePlanningTests(TestCase):
    ...
    def test_full_return_preserves_each_original_technical_row(self) -> None:
        rows = {"POS-ROW-1": [_row("SI-A", 2), _row("SI-B", 3)]}
        planned = consume_return_rows(rows, [_request("POS-ROW-1", 5)], {})
